function sauvegarder(dossierousauver,Stack_name1,Stack_name)
...
for n_start = 1:Stack_length
    ...
    if (minpoints < length(c1) && minpoints < length(c2))
        ...
        if  minpoints < k
            u = c1(1:k);
            v = d1(1:k);
            [m1,~,found1] = pente(u,-v,1,n,dth);
            [u1,v1] = droite(u(1),u(end),-v(1),m1,true);
            v1 = -v1;
            [m2,~,found2] = pente(u,-v,length(u),n,dth);
            [u2,v2] = droite(u(1),u(end),-v(end),m2,false);
            v2 = -v2;
            if (found1 && found2 && (anglecontact(m2,false) < 140))
	            t = t+1;
                theta_a(t) = anglecontact(m1,true);
                theta_r(t) = anglecontact(m2,false);
	            x_a(t) = (xc - u(1)); 
	            y_a(t) = ligne(u(1),v(1));
	            x_r(t) = (xc - u(end));
	            y_r(t) = ligne(u(end),v(end));   
                [vh,ih] = min(v);
	            y_max(t) = ligne(u(ih),vh);
                x_y_max(t) = (xc-u(ih));
